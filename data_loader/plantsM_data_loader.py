...
class DataLoader:
    def __init__(self, config, data_dir):
        ...
        validation_batches = tf.data.experimental.cardinality(self.val_data).numpy()
        val_size = int(0.50 * validation_batches)
        test_size = int(0.50 * validation_batches)
        self.val_data = self.val_data.skip(val_size)
        self.test_data = self.val_data.take(test_size)
    ...
    def preprocess(self):
        self.train_data = self.train_data.map(DataLoader.convert_types)
        self.test_data = self.test_data.map(DataLoader.convert_types)
        self.val_data = self.val_data.map(DataLoader.convert_types)
